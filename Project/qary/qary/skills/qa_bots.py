...
from qary.qary.skills.qa_models import QuestionAnsweringModel
from qary.qary.constants import DATA_DIR, USE_CUDA, MIDATA_URL, MIDATA_QA_MODEL_DIR, args
from qary.qary.skills.basebots import ContextBot
from qary.qary.etl.netutils import DownloadProgressBar
from qary.qary.etl import scrape_wikipedia
...
class Bot(ContextBot):
    ...
    def __init__(self, context=None, args=args, **kwargs):
        ...
        if (
            ...
            (model_type == 'albert' and not os.path.exists(
                os.path.join(model_dir, 'spiece.model')))
        ):
            zip_local_path = os.path.join(
                DATA_DIR, 'qa-models', f"{qa_model}.zip")
            with DownloadProgressBar(unit='B', unit_scale=True, miniters=1, desc=url_str.split('/')[-1]) as t:
                urllib.request.urlretrieve(
                    url_str, filename=zip_local_path, reporthook=t.update_to)
    ...
    def reply(self, statement, context=None, **kwargs):
        ...
        """
        log.warning(
            f"ContextBot.reply(statement={statement}, context={context})")
        ...
        for text in docs:
            log.info(
                f"text from context['doc']['text'] or wikipedia scrape: {text}")
            ...
            if len(response) > 0:
                ...
                if probability > STOP_WIKI_PROBABILITY:
                    log.warning(
                        f"Short circuiting wiki crawl because p > thresh: {probability} > {STOP_WIKI_PROBABILITY}")
                    break
        return responses
